  ...
  if (animationType in staticAnimations) {
    ...
    for (const key in animations.default) {
      if (
        ...
        continue;
      // Only apply static animation variants to path-related elements
      // For other elements (like line, rect), preserve the original variant
      // to maintain required attributes like x1, y1, x2, y2
      if (key.includes('path') || key.includes('Path')) {
        result[key] = variant as T[Extract<keyof T, string>];
      } else {
        // Preserve original variant for non-path elements
        result[key] = animations.default[key] as T[Extract<keyof T, string>];
      }
    }
  } else {
